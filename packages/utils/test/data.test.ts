import { hexToBin, VmNumberError } from '@bitauth/libauth';
import {
  decodeBool,
  decodeInt,
  decodeString,
...
describe('data utils', () => {
  ...
  describe('encodeInt()', () => {
    it('should encode integers', () => {
      expect(encodeInt(0n)).toEqual(hexToBin(''));
      expect(encodeInt(BigInt(-0))).toEqual(hexToBin(''));
      expect(encodeInt(1n)).toEqual(hexToBin('01'));
      expect(encodeInt(BigInt(-1))).toEqual(hexToBin('81'));
      expect(encodeInt(1000n)).toEqual(hexToBin('e803'));
      expect(encodeInt(BigInt(-1000))).toEqual(hexToBin('e883'));
      expect(encodeInt(100000000000000n)).toEqual(hexToBin('00407a10f35a'));
    });
  });

  describe('decodeInt()', () => {
    it('should decode integers', () => {
      expect(decodeInt(hexToBin(''))).toEqual(0n);
      expect(decodeInt(hexToBin('01'))).toEqual(1n);
    ...
    it('should throw if the integer is not minimally encoded', () => {
      const expectedError = VmNumberError.requiresMinimal;
