...
function replaceOps(
  ...
  optimisations.forEach(({ pattern, replacement, patternTokens }) => {
    ...
    while (scriptIndex <= newScript.length - patternLength) {
      ...
      // Source tags use raw script indices (no constructor offset), so we adjust using the script index directly
      // (snapshotted to a const since scriptIndex advances as the scan continues)
      const tagIndex = scriptIndex;
      newSourceTags = newSourceTags.map((tag) => ({
        ...tag,
        startIndex: tag.startIndex >= tagIndex ? Math.max(tagIndex, tag.startIndex - lengthDiff) : tag.startIndex,
        endIndex: tag.endIndex >= tagIndex ? Math.max(tagIndex, tag.endIndex - lengthDiff) : tag.endIndex,
      }));
